...
void on_state_error(void*, char const*, int) noexcept {}
...
// /proc/self/exe or equivalent. threaded=1 enables internal locking. A
// non-null error_callback is required here: backtrace_create_state calls it
// directly (with no null check) when threaded mode isn't supported by the
// build, so passing nullptr would crash instead of gracefully returning
// nullptr as backtrace_create_state otherwise does on error.
...
// Collapse "./" components that appear when libbacktrace joins DW_AT_comp_dir
// with DW_AT_name: Clang records the build directory as comp_dir, which after
// -ffile-prefix-map becomes ".", producing "././example/foo.cpp" for a
// relative DW_AT_name (or just "./example/foo.cpp" for a plain one).
...
struct pcinfo_ctx
{
    ...
    // chain, libbacktrace may call this callback multiple times, reporting
    // the innermost (most specific) location first - the one that matches
    // this particular frame's address.
